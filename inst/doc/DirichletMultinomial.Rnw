...
sense of the model return value is provided by the structure of the
\R{} object \Robject{fit}.
%% 
<<fit>>=
if (full) {
    fit <- mclapply(1:7, dmn, count=count, verbose=TRUE)
    save(fit, file="~/DirichletMultinomial/data/fit.rda")
} else data(fit)
fit[[4]]
names(fit[[4]])
Filter(Negate(is.null), lapply(fit[[4]], names))
@ 
%% 
...
fl <- system.file(package="DirichletMultinomial", "extdata",
                  "TwinStudy.t")
pheno0 <- scan("~/DirichletMultinomial/inst/extdata/TwinStudy.t")
...
if (full) {
    ...
    save(bestgrp, file="~/DirichletMultinomial/data/bestgrp.rda")
} else data(bestgrp)
@ 
%% 
...
if (full) {
    ...
    save(xval, file="~/DirichletMultinomial/data/xval.rda")
} else data(xval)
@ 
%% 
